...
public class Order {
    ...
    //==연관관계 메서드==//
    public void setMember(Member member) {    // 회원 <-> 주문 관계 설정
        this.member = member;
        member.getOrders().add(this);   // member -> order
    }
    
    public void addOrderItem(OrderItem orderItem) {    // 주문 <-> 주문상품 관계 설정
        orderItems.add(orderItem);    // OrderItem은 List이므로 add함수와 setOrder로 연관관계를 구현
        orderItem.setOrder(this);     // orderItem <- order
    }
    
    public void setDelivery(Delivery delivery) {    // 주문 <-> 배달 관계 설정
